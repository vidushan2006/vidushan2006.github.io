/**
 * VIDU AI Assistant 
 * Version: 3.2 (Gemini Enhanced - DOM Load Fix)
 * * This script contains the complete logic for a self-hosted, highly knowledgeable
 * AI assistant for the VIDU website. The entire script is now wrapped in a
 * DOMContentLoaded listener to guarantee all HTML elements are ready before execution.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (SpeechRecognition) {
        ...
        recognition.onend = () => {
            ...
            if(DOMElements.micBtn) DOMElements.micBtn.classList.remove('recording');
        };
    }
...
* **Link Glo-Up (Converter):** Turns long, ugly YouTube links into clean, short \`youtu.be\` links. Perfect for sharing!`,
    ...
    function setupEventListeners() {
        ...
        if (recognition && DOMElements.micBtn) {
    ...
    function handleSendMessage() {
        ...
        setTimeout(() => {
            ...
            if (DOMElements.historyContainer) {
    ...
    function addMessage(role, text) {
        ...
        const contentHtml = (typeof marked !== 'undefined' && !(role === 'model' && text.includes('typing-indicator'))) 
            ? marked.parse(text) 
            : text;
    ...
    function toggleSpeechRecognition() {
        ...
        if (state.isRecognizing) {
            ...
            if (DOMElements.micBtn) DOMElements.micBtn.classList.add('recording');
            recognition.start();
        }
    }
